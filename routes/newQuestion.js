...
router.get("/new-question", csrfProtection, function (req, res, next) {
    res.render('newQuestion', {csrfToken: req.csrfToken(), bodyVal: "", titleVal: ""})
})

router.post("/new-question", csrfProtection, asyncHandler(async (req, res, next) => {
    const errors = [];
    if(!req.body.title) {
    ...
    if(errors.length === 0) {
        ...
        res.render('newQuestion', {errors, csrfToken: req.csrfToken(), bodyVal: req.body.body, titleVal: req.body.title})
    }
}))
